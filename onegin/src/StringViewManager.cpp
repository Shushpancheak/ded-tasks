...
void StringViewManager::SortFromLeft() {
	std::sort(string_views_, string_views_ + string_count_, CompareStringViewsFromLeft);
}

// ReSharper disable once CppMemberFunctionMayBeConst
void StringViewManager::SortFromRight() {
	std::sort(string_views_, string_views_ + string_count_, CompareStringViewsFromRight);
}
...
bool StringViewManager::CompareStringViewsFromLeft(const std::string_view& s1, const std::string_view& s2) {
	size_t i = 0;
	size_t j = 0;
	for (; i < s1.length() && j < s2.length(); ++i, ++j) {
		// Iterating to next alpha symbol.
		while (!isalpha(static_cast<unsigned char>(s1[i])) &&
						!isdigit(static_cast<unsigned char>(s1[i]))) {
			++i;
			if (i >= s1.length() - 1) {
				i = s1.length() - 1;
				break;
		...
		while (!isalpha(static_cast<unsigned char>(s2[j])) &&
						!isdigit(static_cast<unsigned char>(s2[j]))) {
			++j;
			if (j >= s2.length() - 1) {
				j = s2.length() - 1;
				break;
...
bool StringViewManager::CompareStringViewsFromRight(const std::string_view& s1, const std::string_view& s2) {
	int i = s1.length() - 1;
	int j = s2.length() - 1;
	for (; i >= 0 && j >= 0; --i, --j) {
		// Iterating to next alpha symbol.
